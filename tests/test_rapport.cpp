...
private slots:
	...
	void calc_rapport_finds_simple_warp_repeat()
	{
		...
		/*  clear gewebe so EinzugEqual's schuss-comparison is always
		    equal (rows of zeros). */
		for (int i = 0; i < 8; i++) DBWFRM->gewebe.feld.Set(i, 0, (char)0);
	...
	void calc_rapport_respects_overridden_flag()
	{
		...
		for (int i = 0; i < 4; i++) DBWFRM->einzug.feld.Set(i, (short)(i + 1));
		DBWFRM->CalcRapport();
	...
	void calc_rapport_entire_range_when_not_repeating()
	{
		...
		for (int i = 0; i < 5; i++) DBWFRM->einzug.feld.Set(i, (short)(i + 1));
		DBWFRM->CalcRapport();
		QCOMPARE(DBWFRM->rapport.kr.a, 0);
