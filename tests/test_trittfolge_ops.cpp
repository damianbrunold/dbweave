...
private slots:
	...
	void is_empty_tritt_reads_freietritte()
	{
	...
	{
		DBWFRM->freietritte[3] = false;
		DBWFRM->freietritte[7] = false;
	...
	void move_tritt_copies_trittfolge_and_aufknuepfung_and_flips_free()
	{
		...
		QCOMPARE(DBWFRM->freietritte[2], true);
		QCOMPARE(DBWFRM->freietritte[0], false);
	}
	...
	void rearrange_tritte_minimalz_compacts_from_left()
	{
		...
		for (int i = 0; i < Data->MAXX2; i++)
			if (DBWFRM->trittfolge.feld.Get(i, 0) != 0 ||
			    DBWFRM->trittfolge.feld.Get(i, 1) != 0)
				count_nonzero++;
	...
	{
		/*  Shaft 3 has no einzug -> freieschaefte[3] = true (default).
	...
	void copy_schussfaden_duplicates_trittfolge_and_gewebe_row()
	{
		...
		QCOMPARE(DBWFRM->trittfolge.isempty.Get(8), false);
