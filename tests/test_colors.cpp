...
private slots:
	...
	{
		int r, g, b;
		HSV2RGB(  0.0f, 1.0f, 1.0f, r, g, b); QCOMPARE(r, 255); QCOMPARE(g,   0); QCOMPARE(b,   0);
		HSV2RGB(120.0f, 1.0f, 1.0f, r, g, b); QCOMPARE(r,   0); QCOMPARE(g, 255); QCOMPARE(b,   0);
		HSV2RGB(240.0f, 1.0f, 1.0f, r, g, b); QCOMPARE(r,   0); QCOMPARE(g,   0); QCOMPARE(b, 255);
	}
	...
	void round_trip_rgb_hsv_rgb()
	{
		...
		for (auto c : cases) {
			...
			int r, g, b;
			HSV2RGB(h, s, v, r, g, b);
			QVERIFY2(std::abs(r - c.r) <= 1, qPrintable(QString("r: got %1, want %2").arg(r).arg(c.r)));
			QVERIFY2(std::abs(g - c.g) <= 1, qPrintable(QString("g: got %1, want %2").arg(g).arg(c.g)));
			QVERIFY2(std::abs(b - c.b) <= 1, qPrintable(QString("b: got %1, want %2").arg(b).arg(c.b)));
		}
	}
};
