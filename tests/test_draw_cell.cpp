...
class TestDrawCell : public QObject
{
	...
	static QImage render (DARSTELLUNG d, int number = -1)
	{
		...
		img.fill(Qt::white);
		QPainter p(&img);
		PaintCell(p, d, 0, 0, SZ, SZ, QColor(Qt::black), /*dontclear=*/false,
		          number, QColor(Qt::white));
		return img;
	}
...
private slots:
	...
	void kreuz_paints_two_diagonals()
	{
		...
		QVERIFY2(fg > 2 * (SZ - 4),
		         qPrintable(QString("expected at least %1 FG pixels, got %2").arg(2*(SZ-4)).arg(fg)));
