...
WEAVE_STATUS StSlipsController::WeaveSchuss(std::uint32_t _shafts)
{
    ...
    if (repeat)
        return forward ? WEAVE_SUCCESS_PREV : WEAVE_SUCCESS_NEXT;
...
std::unique_ptr<StWeaveController> makeLoomController(LOOMINTERFACE _intrf)
{
    ...
    case intrf_arm_patronic:
        return std::make_unique<StPatronicController>();
    case intrf_arm_patronic_indirect:
        return std::make_unique<StPatronicIndirectController>();
    case intrf_arm_designer:
        return std::make_unique<StDesignerController>();
    case intrf_slips:
        return std::make_unique<StSlipsController>();
    case intrf_avl_cd_iii:
        return std::make_unique<StAvlCdIIIController>();
    default:
