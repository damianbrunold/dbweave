...
bool SerialPort::Open (PORT _port, const PORTINIT& _init)
{
	...
	case BR_1200:  br = QSerialPort::Baud1200;  break;
	case BR_2400:  br = QSerialPort::Baud2400;  break;
	case BR_4800:  br = QSerialPort::Baud4800;  break;
	case BR_9600:  br = QSerialPort::Baud9600;  break;
	case BR_14400: br = QSerialPort::BaudRate(14400); break;
	}
	port->setBaudRate(br);
	...
	case SB_ONE:     sb = QSerialPort::OneStop;        break;
	case SB_ONEFIVE: sb = QSerialPort::OneAndHalfStop; break;
	case SB_TWO:     sb = QSerialPort::TwoStop;        break;
	}
	port->setStopBits(sb);

	QSerialPort::Parity pa = QSerialPort::NoParity;
	switch (_init.parity) {
	case PA_NONE: pa = QSerialPort::NoParity;   break;
	case PA_ODD:  pa = QSerialPort::OddParity;  break;
	case PA_EVEN: pa = QSerialPort::EvenParity; break;
	}
	port->setParity(pa);
...
bool SerialPort::IsOpen () const { return port && port->isOpen(); }
...
{
	if (!port || !port->isOpen() || !_buffer) return false;
...
bool SerialPort::Send (const char* _buffer, int _length)
{
	...
	if (written != _length) return false;
	return port->waitForBytesWritten(2000);
}
...
{
	if (!_buffer || _length <= 0) return false;
...
{
	if (rxBuf.isEmpty()) drainInto(50);
	if (rxBuf.isEmpty()) return '\0';
