    ...
    : QDialog(_parent)
{
    ...
    line(QStringLiteral("Kernel: %1 %2").arg(QSysInfo::kernelType(), QSysInfo::kernelVersion()));
    ...
    line(QStringLiteral("Byte order: %1-endian")
             .arg(QSysInfo::ByteOrder == QSysInfo::LittleEndian ? QStringLiteral("little")
                                                                : QStringLiteral("big")));
    ...
    if (QScreen* s = QGuiApplication::primaryScreen()) {
        ...
        const quint64 cols = (depth >= 32) ? (quint64(1) << 32)
                             : (depth > 0) ? (quint64(1) << depth)
                                           : 0;
        if (cols)
            line(QStringLiteral("%1 colors").arg(cols));
    }
