...
void TDBWFRM::PasteSelection(bool _transparent)
{
    ...
    case GEWEBE: {
        ...
        if (cursorhandler)
            cursorhandler->SetCursor(GEWEBE, start.i + x, start.j, true);
        break;
    }
    case EINZUG: {
        for (int j = y - 1; j >= 0; j--) {
        ...
        if (cursorhandler)
            cursorhandler->SetCursor(EINZUG, start.i + x, start.j, true);
        break;
    }
    case TRITTFOLGE: {
        for (int j = y - 1; j >= 0; j--) {
        ...
        if (cursorhandler)
            cursorhandler->SetCursor(TRITTFOLGE, start.i, start.j + y, true);
        break;
    }
    case AUFKNUEPFUNG: {
        for (int j = y - 1; j >= 0; j--) {
        ...
        if (cursorhandler)
            cursorhandler->SetCursor(AUFKNUEPFUNG, start.i + x, start.j, true);
        break;
    }
    default:
        return;
...
void TDBWFRM::InvertSelection()
{
    ...
    case TRITTFOLGE: {
        ...
        if (singleTritt)
            break; /* legacy rings the bell and no-ops */
        for (int j = selection.begin.j; j <= selection.end.j; j++)
            for (int i = selection.begin.i; i <= selection.end.i; i++) {
...
void TDBWFRM::MirrorHorzSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        mirrorRow([this](int i, int j) { return gewebe.feld.Get(i, j); },
                  [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        mirrorRow([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
...
void TDBWFRM::MirrorVertSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        mirrorCol([this](int i, int j) { return gewebe.feld.Get(i, j); },
                  [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        mirrorCol([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
                  [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case EINZUG:
...
void TDBWFRM::RotateSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        rotate([this](int i, int j) { return gewebe.feld.Get(i, j); },
               [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        rotate([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
               [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case TRITTFOLGE: {
...
void TDBWFRM::RollUpSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        rollUp([this](int i, int j) { return gewebe.feld.Get(i, j); },
               [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        rollUp([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
               [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case EINZUG: {
        ...
        for (int i = selection.begin.i; i <= selection.end.i; i++) {
            ...
            if (s)
                einzug.feld.Set(
                    i, short(selection.begin.j + 1 + ((s - 1 - selection.begin.j + 1) % dy)));
        }
        RecalcGewebe();
        break;
    }
    case TRITTFOLGE:
        rollUp([this](int i, int j) { return trittfolge.feld.Get(i, j); },
               [this](int i, int j, char v) { trittfolge.feld.Set(i, j, v); });
...
void TDBWFRM::RollDownSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        rollDown([this](int i, int j) { return gewebe.feld.Get(i, j); },
                 [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        rollDown([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
                 [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case EINZUG: {
        ...
        for (int i = selection.begin.i; i <= selection.end.i; i++) {
            ...
            if (s)
                einzug.feld.Set(
                    i, short(selection.begin.j + 1 + ((s - 1 - selection.begin.j - 1 + dy) % dy)));
        }
        RecalcGewebe();
        break;
    }
    case TRITTFOLGE:
        rollDown([this](int i, int j) { return trittfolge.feld.Get(i, j); },
                 [this](int i, int j, char v) { trittfolge.feld.Set(i, j, v); });
        rollTrittfolgeEmpty(this, selection.begin.j, selection.end.j);
        RecalcGewebe();
        break;
...
void TDBWFRM::RollLeftSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        rollLeft([this](int i, int j) { return gewebe.feld.Get(i, j); },
                 [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        rollLeft([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
                 [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case EINZUG: {
    ...
    case TRITTFOLGE:
        rollLeft([this](int i, int j) { return trittfolge.feld.Get(i, j); },
                 [this](int i, int j, char v) { trittfolge.feld.Set(i, j, v); });
        rollTrittfolgeEmpty(this, selection.begin.j, selection.end.j);
        RecalcGewebe();
        break;
...
void TDBWFRM::RollRightSelection()
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        rollRight([this](int i, int j) { return gewebe.feld.Get(i, j); },
                  [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        rollRight([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
                  [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case EINZUG: {
    ...
    case TRITTFOLGE:
        rollRight([this](int i, int j) { return trittfolge.feld.Get(i, j); },
                  [this](int i, int j, char v) { trittfolge.feld.Set(i, j, v); });
        rollTrittfolgeEmpty(this, selection.begin.j, selection.end.j);
        RecalcGewebe();
        break;
...
void TDBWFRM::CentralsymSelection()
{
    ...
    const bool singleTritt
        = trittfolge.einzeltritt && !(ViewSchlagpatrone && ViewSchlagpatrone->isChecked());
    if (!(selection.feld == GEWEBE || selection.feld == AUFKNUEPFUNG
    ...
    switch (selection.feld) {
    case GEWEBE:
        copyIn([this](int i, int j) { return gewebe.feld.Get(i, j); });
        break;
    case AUFKNUEPFUNG:
        copyIn([this](int i, int j) { return aufknuepfung.feld.Get(i, j); });
        break;
    case TRITTFOLGE:
        copyIn([this](int i, int j) { return trittfolge.feld.Get(i, j); });
        break;
    default:
        selection = savesel;
        return;
...
void TDBWFRM::SwitchRange(int _range)
{
    ...
    switch (selection.feld) {
    case GEWEBE:
        switchField([this](int i, int j) { return gewebe.feld.Get(i, j); },
                    [this](int i, int j, char v) { gewebe.feld.Set(i, j, v); });
        break;
    case AUFKNUEPFUNG:
        switchField([this](int i, int j) { return aufknuepfung.feld.Get(i, j); },
                    [this](int i, int j, char v) { aufknuepfung.feld.Set(i, j, v); });
        RecalcGewebe();
        break;
    case TRITTFOLGE:
