...
protected:
    ...
    void mousePressEvent(QMouseEvent* _e) override
    {
        ...
        if (i < 0 || i >= kCols || j < 0 || j >= kRows)
            return;
        if (i + j * kCols >= MAX_PAL_ENTRY)
            return;
        setXY(i, j);
        setFocus();
        update();
        ...
        case Qt::Key_Down:
            ...
            if (y >= kRows) {
                if (x < kCols) {
                    ...
                    y = kRows - 1;
            } else if (x + y * kCols >= MAX_PAL_ENTRY && x < kCols) {
                x++;
                y = 0;
            }
            break;
...
int SelColorDialog::selectedIndex() const
{
    return canvas->selcolor;
}
