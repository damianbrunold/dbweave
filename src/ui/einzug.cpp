...
void EinzugRearrangeImpl::Rearrange()
{
	...
	else if (frm->EzMinimalS && frm->EzMinimalS->isChecked()) NormalS();
	else if (frm->EzGeradeZ  && frm->EzGeradeZ->isChecked())  GeradeZ();
	else if (frm->EzGeradeS  && frm->EzGeradeS->isChecked())  GeradeS();
	else if (frm->EzChorig2  && frm->EzChorig2->isChecked())  Chorig2();
	else if (frm->EzChorig3  && frm->EzChorig3->isChecked())  Chorig3();
...
void EinzugRearrangeImpl::RedrawSchlagpatrone (int _i)
{
	...
	if (_i<frm->scroll_x2 || _i>=frm->scroll_x2+frm->trittfolge.pos.width/frm->trittfolge.gw) return;
		...
		if (IsTotalEmptySchaft(j)) {
			for (int i=0; i<Data->MAXX1; i++)
				if (frm->einzug.feld.Get(i)==j+1) {
					...
					if (frm->einzug.gw>0 && i>=frm->scroll_x1 && i<frm->scroll_x1+frm->einzug.pos.width/frm->einzug.gw)
			...
			if (!frm->ViewSchlagpatrone || !frm->ViewSchlagpatrone->isChecked()) {
				for (int i=0; i<Data->MAXX2; i++)
					if (frm->aufknuepfung.feld.Get(i, j)>0) {
						...
						if (frm->aufknuepfung.gw>0 && i>=frm->scroll_x2 && i<frm->scroll_x2+frm->aufknuepfung.pos.width/frm->aufknuepfung.gw)
			...
			if (firstnonempty==-1 || firstnonempty<j) {
				if (!frm->ViewSchlagpatrone || !frm->ViewSchlagpatrone->isChecked()) {
					...
					for (int i=0; i<Data->MAXY2; i++) {
						...
						if (frm->trittfolge.gh>0 && i>=frm->scroll_y2 && i<frm->scroll_y2+frm->trittfolge.pos.height/frm->trittfolge.gh)
...
int EinzugRearrangeImpl::SplitSchaft (int _searchj, int _sourcej)
{
	...
	if (!frm->ViewSchlagpatrone || !frm->ViewSchlagpatrone->isChecked()) RedrawAufknuepfungSchaft (_sourcej);
