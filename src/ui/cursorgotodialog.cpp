	...
	: QDialog(_parent)
{
	...
	rTrittfolge    = new QRadioButton(_pegplan
	                    ? QStringLiteral("Pegp&lan")
	                    : QStringLiteral("Tre&adling"),              this);
	...
	/*  Legacy: in pegplan (ViewSchlagpatrone) mode the
	    ...
	    disabled. */
	if (_pegplan) rAufknuepfung->setEnabled(false);

	switch (_initial) {
	case EINZUG:       rEinzug      ->setChecked(true); break;
	case TRITTFOLGE:   rTrittfolge  ->setChecked(true); break;
	case AUFKNUEPFUNG: rAufknuepfung->setChecked(true); break;
	case KETTFARBEN:   rKettfarben  ->setChecked(true); break;
	case SCHUSSFARBEN: rSchussfarben->setChecked(true); break;
	case GEWEBE:
	default:           rBindung     ->setChecked(true); break;
	}
...
{
	if (rEinzug      ->isChecked()) return EINZUG;
	if (rTrittfolge  ->isChecked()) return TRITTFOLGE;
	if (rAufknuepfung->isChecked()) return AUFKNUEPFUNG;
	if (rKettfarben  ->isChecked()) return KETTFARBEN;
	if (rSchussfarben->isChecked()) return SCHUSSFARBEN;
	return GEWEBE;
}
