...
void PrPrinter::CalcBaseDimensions()
{
    ...
    y_footer
        = pageHeight() - (mainfrm->borders.range.bottom + mainfrm->footer.height) * 10 * sy / 254;
    ...
    if (mainfrm->faktor_schuss > mainfrm->faktor_kette) {
        const int gridheight
            = gh * int(double(zoomCur) * mainfrm->faktor_schuss / mainfrm->faktor_kette) / zoomBase;
        einzug.gw = gwidth;
        einzug.gh = gheight;
        ...
        const int gridwidth
            = gw * int(double(zoomCur) * mainfrm->faktor_kette / mainfrm->faktor_schuss) / zoomBase;
        einzug.gw = gridwidth;
        einzug.gh = gheight;
        aufknuepfung.gw = gwidth;
        aufknuepfung.gh = gheight;
...
void PrPrinter::CalcDimensions()
{
    ...
    if (mainfrm->einzugunten) {
        ...
        if (aufknuepfung.visible)
            trittfolge.y0 += yabstand;
        if (blatteinzug.visible
            || (mainfrm->ViewBlatteinzug && mainfrm->ViewBlatteinzug->isChecked()
                && aufknuepfung.visible))
...
void PrPrinter::InitVisibility()
{
    ...
    if (viewFarbe && kette.count() > 0)
        kettfarben.visible = true;
    if (viewEinzug && kette.count() > 0 && schaefte.count() > 0)
        einzug.visible = true;
    if (viewBlatt && kette.count() > 0)
        blatteinzug.visible = true;
    if (kette.count() > 0 && schuesse.count() > 0)
        gewebe.visible = true;
    if (viewTritt && schuesse.count() > 0 && tritte.count() > 0)
        trittfolge.visible = true;
    if (viewEinzug && viewTritt && schaefte.count() > 0 && tritte.count() > 0)
        aufknuepfung.visible = true;
    if (viewFarbe && schuesse.count() > 0)
...
bool PrPrinter::ClipToPage(int _page)
{
    ...
    if (i2 > 0) {
        if (schussfarben.visible) {
            if (i2 >= 2 * schussfarben.gw)
                i2 -= 2 * schussfarben.gw;
            else
                i2 = 0;
