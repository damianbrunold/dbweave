...
void PrPrinter::SetExtent(int _w, int _h)
{
    cwidth = _w;
    cheight = _h;
}
void PrPrinter::SetScale(double _xs, double _ys)
{
    x_scale = _xs;
    y_scale = _ys;
}

int PrPrinter::dpiX() const
{
    return device ? device->logicalDpiX() : 96;
}
int PrPrinter::dpiY() const
{
    return device ? device->logicalDpiY() : 96;
}
...
void PrPrinter::Print(int _von, int _bis)
{
    ...
    while ((_bis == -1 || pagenr <= _bis)
           && (failed = !PrintPage(pagenr, pagenr > _von)) == false) {
        ++pagenr;
        if (_bis != -1 && pagenr > _bis)
            break;
        if ((pagenr - _von) >= pages)
            break;
    }
...
/*-----------------------------------------------------------------*/
void PrPrinter::PrintRange(const SZ& _kette, const SZ& _schuesse, const SZ& _schaefte,
                           const SZ& _tritte)
{
    kette = _kette;
    schuesse = _schuesse;
...
void PrPrinter::PrepareHeader()
{
    header = expandTokens(mainfrm->header.text, mainfrm, pagenr);
}
void PrPrinter::PrepareFooter()
{
    footer = expandTokens(mainfrm->footer.text, mainfrm, pagenr);
}
