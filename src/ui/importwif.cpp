...
struct WifWeaving   { int shafts = 0; int treadles = 0; bool risingshed = false; };
struct WifWeftWarp  { int threads = 0; int color = 1; };
struct WifColorPalette {
	int entries = 0;
	int beginrange = 0;
	int endrange = 65535;
};
struct WifThreadingEntry  { QVector<int> harnesses; };
struct WifTieupEntry      { QVector<int> harnesses; };
struct WifTreadlingEntry  { QVector<int> treadles; };

class WifReader
{
public:
	explicit WifReader (TDBWFRM* _frm) : frm(_frm) {}
...
bool WifReader::fieldBool (const QString& _line)
{
	...
	return v == QStringLiteral("TRUE") || v == QStringLiteral("YES")
	    || v == QStringLiteral("1")    || v == QStringLiteral("ON");
}
...
QVector<int> WifReader::splitInts (const QString& _line)
{
	...
	for (const QString& p : parts) out.append(p.trimmed().toInt());
	return out;
}
...
static QString readWeaving (QTextStream& _s, WifWeaving& _w)
{
	while (!_s.atEnd()) {
		...
		if (t.startsWith('[')) return t.toLower();
		const QString f = WifReader::fieldName(line);
		if      (f == "shafts")       _w.shafts      = WifReader::fieldInt(line);
		else if (f == "treadles")     _w.treadles    = WifReader::fieldInt(line);
		else if (f == "rising shed")  _w.risingshed  = WifReader::fieldBool(line);
	}
	return QString();
}
...
static QString readWeftWarp (QTextStream& _s, WifWeftWarp& _w)
{
	while (!_s.atEnd()) {
		...
		if (t.startsWith('[')) return t.toLower();
		const QString f = WifReader::fieldName(line);
		if      (f == "color")   _w.color   = WifReader::fieldInt(line);
		else if (f == "threads") _w.threads = WifReader::fieldInt(line);
	}
	return QString();
}
...
static QString readColorPalette (QTextStream& _s, WifColorPalette& _p)
{
	while (!_s.atEnd()) {
		...
		if (t.startsWith('[')) return t.toLower();
		const QString f = WifReader::fieldName(line);
		if (f == "entries") _p.entries = WifReader::fieldInt(line);
		else if (f == "range") {
			const QVector<int> r = WifReader::splitInts(WifReader::fieldValue(line));
			if (r.size() >= 2) { _p.beginrange = r[0]; _p.endrange = r[1]; }
		}
	}
	return QString();
}
...
static QString readIndexed (QTextStream& _s, QVector<Entry>& _entries, Assign _assign)
{
	while (!_s.atEnd()) {
		...
		if (n < 0 || n >= _entries.size()) continue;
		Entry entry;
...
void WifReader::firstPass ()
{
	...
	while (!tok.isEmpty()) {
		if      (tok == "[weaving]")       tok = readWeaving(s, weaving);
		else if (tok == "[color palette]") tok = readColorPalette(s, colorpalette);
		else                                tok = nextToken(s);
	}
}
...
void WifReader::secondPass ()
{
	...
	QString tok = nextToken(s);
	while (!tok.isEmpty()) {
		if      (tok == "[weft]") tok = readWeftWarp(s, weft);
		else if (tok == "[warp]") tok = readWeftWarp(s, warp);
		else                       tok = nextToken(s);
	}
}
...
void WifReader::copyData ()
{
	...
	for (int i = 0; i < threading.size(); i++) {
		if (threading[i].harnesses.isEmpty()) continue;
...
void TDBWFRM::ImportWIFClick ()
{
	const QString fn = QFileDialog::getOpenFileName(
	    this,
	    QStringLiteral("Import WIF file"),
	    QString(),
	...
	for (int i = 0; i < Data->MAXX1; i++) {
		...
		if (s > 0 && s - 1 < Data->MAXY1) freieschaefte[s - 1] = false;
	}
	for (int i = 0; i < Data->MAXX2; i++) {
		bool used = false;
		for (int j = 0; j < Data->MAXY1 && !used; j++)
			if (aufknuepfung.feld.Get(i, j) > 0) used = true;
		freietritte[i] = !used;
	}
	if (undo) { undo->Clear(); undo->Snapshot(); }
	refresh();
}
