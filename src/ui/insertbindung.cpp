...
void __fastcall TDBWFRM::AtlasEinfuegen (int _n)
{
	...
	switch (_n) {
	case 5:
		FSet(m, posi+0, posj+0, s); FSet(m, posi+1, posj+2, s);
		FSet(m, posi+2, posj+4, s); FSet(m, posi+3, posj+1, s);
		FSet(m, posi+4, posj+3, s);
		break;
	case 6:
		FSet(m, posi+0, posj+0, s); FSet(m, posi+1, posj+2, s);
		FSet(m, posi+2, posj+4, s); FSet(m, posi+3, posj+1, s);
		FSet(m, posi+4, posj+5, s); FSet(m, posi+5, posj+3, s);
		break;
	case 7:
		FSet(m, posi+0, posj+2, s); FSet(m, posi+1, posj+6, s);
		FSet(m, posi+2, posj+3, s); FSet(m, posi+3, posj+0, s);
		FSet(m, posi+4, posj+4, s); FSet(m, posi+5, posj+1, s);
		FSet(m, posi+6, posj+5, s);
		break;
	case 8:
		FSet(m, posi+0, posj+2, s); FSet(m, posi+1, posj+5, s);
		FSet(m, posi+2, posj+0, s); FSet(m, posi+3, posj+3, s);
		FSet(m, posi+4, posj+6, s); FSet(m, posi+5, posj+1, s);
		FSet(m, posi+6, posj+4, s); FSet(m, posi+7, posj+7, s);
		break;
	case 9:
		FSet(m, posi+0, posj+0, s); FSet(m, posi+1, posj+2, s);
		FSet(m, posi+2, posj+4, s); FSet(m, posi+3, posj+6, s);
		FSet(m, posi+4, posj+8, s); FSet(m, posi+5, posj+1, s);
		FSet(m, posi+6, posj+3, s); FSet(m, posi+7, posj+5, s);
		FSet(m, posi+8, posj+7, s);
		break;
	case 10:
		FSet(m, posi+0, posj+0, s); FSet(m, posi+1, posj+7, s);
		FSet(m, posi+2, posj+4, s); FSet(m, posi+3, posj+1, s);
		FSet(m, posi+4, posj+8, s); FSet(m, posi+5, posj+5, s);
		FSet(m, posi+6, posj+2, s); FSet(m, posi+7, posj+9, s);
		FSet(m, posi+8, posj+6, s); FSet(m, posi+9, posj+3, s);
		break;
	}
