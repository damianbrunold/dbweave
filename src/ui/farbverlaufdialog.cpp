...
{
public:
    BlendStrip(FarbverlaufDialog* _parent, const COLORREF* _table, const int* _count,
               const COLORREF* _start, const COLORREF* _end, const bool* _noDividers)
        : QWidget(_parent)
        , table(_table)
        , countPtr(_count)
        , startPtr(_start)
    ...
    , frm(_frm)
{
    ...
    connect(edSteps, qOverload<int>(&QSpinBox::valueChanged), this, [this] {
        createFarbverlauf();
        repaintBlend();
    });
    connect(slWeight, &QSlider::valueChanged, this, [this] {
        createFarbverlauf();
        repaintBlend();
    });
    ...
    connect(modelRGB, &QRadioButton::toggled, this, [this] {
        createFarbverlauf();
        repaintBlend();
    });
    connect(modelHSV, &QRadioButton::toggled, this, [this] {
        createFarbverlauf();
        repaintBlend();
    });
...
{
    if (auto* s = static_cast<Swatch*>(_w))
        s->setColor(_col);
}
...
void FarbverlaufDialog::createFarbverlauf()
{
    ...
    if (modelRGB->isChecked())
        farbverlaufRGB(abstufungen);
    else
        farbverlaufHSV(abstufungen);
}
...
void FarbverlaufDialog::farbverlaufHSV(int _abstufungen)
{
    ...
    for (int i = 0; i < _abstufungen; i++) {
        ...
        QColor c
            = QColor::fromHsvF(std::max(0.f, std::min(1.f, h)), std::max(0.f, std::min(1.f, s)),
