...
static void calcSelectionRect(TDBWFRM* frm, int& _x, int& _y, int& _xx, int& _yy)
{
    ...
    case EINZUG: {
        if (frm->toptobottom) {
            ...
            _y = frm->einzug.pos.y0 + frm->einzug.pos.height
                 - (sel.end.j - frm->scroll_y1 + 1) * frm->einzug.gh;
            _yy = frm->einzug.pos.y0 + frm->einzug.pos.height
                  - (sel.begin.j - frm->scroll_y1) * frm->einzug.gh;
        }
        if (frm->righttoleft) {
            _x = frm->einzug.pos.x0 + frm->einzug.pos.width
                 - (sel.begin.i - frm->scroll_x1) * frm->einzug.gw;
            _xx = frm->einzug.pos.x0 + frm->einzug.pos.width
                  - (sel.end.i - frm->scroll_x1 + 1) * frm->einzug.gw;
        } else {
    ...
    case AUFKNUEPFUNG: {
        if (frm->toptobottom) {
            ...
            _y = frm->aufknuepfung.pos.y0 + frm->aufknuepfung.pos.height
                 - (sel.end.j - frm->scroll_y1 + 1) * frm->aufknuepfung.gh;
            _yy = frm->aufknuepfung.pos.y0 + frm->aufknuepfung.pos.height
                  - (sel.begin.j - frm->scroll_y1) * frm->aufknuepfung.gh;
        }
    ...
    case TRITTFOLGE:
        ...
        _y = frm->trittfolge.pos.y0 + frm->trittfolge.pos.height
             - (sel.end.j - frm->scroll_y2 + 1) * frm->trittfolge.gh;
        _xx = frm->trittfolge.pos.x0 + (sel.end.i - frm->scroll_x2 + 1) * frm->trittfolge.gw;
        _yy = frm->trittfolge.pos.y0 + frm->trittfolge.pos.height
              - (sel.begin.j - frm->scroll_y2) * frm->trittfolge.gh;
        break;
    case GEWEBE: {
        if (frm->righttoleft) {
            _x = frm->gewebe.pos.x0 + frm->gewebe.pos.width
                 - (sel.begin.i - frm->scroll_x1) * frm->gewebe.gw;
            _xx = frm->gewebe.pos.x0 + frm->gewebe.pos.width
                  - (sel.end.i - frm->scroll_x1 + 1) * frm->gewebe.gw;
        } else {
        ...
        _y = frm->gewebe.pos.y0 + frm->gewebe.pos.height
             - (sel.end.j - frm->scroll_y2 + 1) * frm->gewebe.gh;
        _yy = frm->gewebe.pos.y0 + frm->gewebe.pos.height
              - (sel.begin.j - frm->scroll_y2) * frm->gewebe.gh;
        break;
    }
    case BLATTEINZUG:
        ...
        if (frm->righttoleft) {
            _x = frm->blatteinzug.pos.x0 + frm->blatteinzug.pos.width
                 - (sel.begin.i - frm->scroll_x1) * frm->blatteinzug.gw;
            _xx = frm->blatteinzug.pos.x0 + frm->blatteinzug.pos.width
                  - (sel.end.i - frm->scroll_x1 + 1) * frm->blatteinzug.gw;
        } else {
    ...
    case KETTFARBEN:
        ...
        if (frm->righttoleft) {
            _x = frm->kettfarben.pos.x0 + frm->kettfarben.pos.width
                 - (sel.begin.i - frm->scroll_x1) * frm->kettfarben.gw;
            _xx = frm->kettfarben.pos.x0 + frm->kettfarben.pos.width
                  - (sel.end.i - frm->scroll_x1 + 1) * frm->kettfarben.gw;
        } else {
    ...
    case SCHUSSFARBEN:
        ...
        _y = frm->schussfarben.pos.y0 + frm->schussfarben.pos.height
             - (sel.end.j - frm->scroll_y2 + 1) * frm->schussfarben.gh;
        _xx = frm->schussfarben.pos.x0 + frm->schussfarben.pos.width;
        _yy = frm->schussfarben.pos.y0 + frm->schussfarben.pos.height
              - (sel.begin.j - frm->scroll_y2) * frm->schussfarben.gh;
        break;
    default:
        _x = _y = _xx = _yy = 0;
    }
...
void TDBWFRM::DrawSelection()
{
    ...
    if (!selection.Valid())
        return;
    if (selection.feld == GEWEBE && GewebeNone && GewebeNone->isChecked())
        return;
...
void TDBWFRM::ResizeSelection(int _i, int _j, FELD _feld, bool _square)
{
    ...
    if (_square) {
        ...
        const int d = di - dj;
        if (d > 0) {
            if (_i > selection.begin.i)
                _i -= d;
            else
                _i += d;
        } else if (d < 0) {
            if (_j > selection.begin.j)
                _j += d;
            else
                _j -= d;
        }
...
void TDBWFRM::ApplyRangeToSelection(int _range)
{
    ...
    case TRITTFOLGE:
        ...
        break;
    default:
        return; /* strips don't use digit-range apply */
    }
    ...
    for (int i = sel.begin.i; i <= sel.end.i; i++) {
        for (int j = sel.begin.j; j <= sel.end.j; j++) {
            switch (f) {
            case GEWEBE:
                SetGewebe(i - sx, j - sy, /*_set=*/true, _range);
                break;
            case EINZUG:
                SetEinzug(i - sx, j - sy);
                break;
            case AUFKNUEPFUNG:
                SetAufknuepfung(i - sx, j - sy, /*_set=*/true, _range);
                break;
            case TRITTFOLGE:
                SetTrittfolge(i - sx, j - sy, /*_set=*/true, _range);
                break;
            default:
                break;
            }
