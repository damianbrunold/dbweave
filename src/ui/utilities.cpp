...
void TDBWFRM::FillKoeper()
{
    ...
    if (selection.begin.i == selection.end.i) {
        ...
        if (iEnd >= iStart) {
            ...
            CalcRangeSchuesse();
            CalcRapport();
            selection.end.i = std::min(
                iEnd, scroll_x1 + gewebe.pos.width / (gewebe.gw > 0 ? gewebe.gw : 1) - 1);
            SetModified();
            refresh();
            if (undo)
                undo->Snapshot();
        ...
        if (jEnd >= jStart) {
            ...
            CalcRangeSchuesse();
            CalcRapport();
            selection.end.j = std::min(
                jEnd, scroll_y2 + gewebe.pos.height / (gewebe.gh > 0 ? gewebe.gh : 1) - 1);
            SetModified();
            refresh();
            if (undo)
                undo->Snapshot();
...
void TDBWFRM::KettLancierungClick()
{
    ...
    if (a + needed >= Data->MAXX1)
        needed = Data->MAXX1 - 1 - a;
...
void TDBWFRM::SchussLancierungClick()
{
    ...
    if (a + needed >= Data->MAXY2)
        needed = Data->MAXY2 - 1 - a;
