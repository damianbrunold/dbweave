	...
	, cursordirection(_cd)
{
	...
	connect(btns, &QDialogButtonBox::accepted, this, [this] { readFromButtons(); accept(); });
...
void CursorDirDialog::readFromButtons()
{
	...
	if (cdUp   ->isChecked()) cursordirection |= CD_UP;
	if (cdDown ->isChecked()) cursordirection |= CD_DOWN;
	if (cdLeft ->isChecked()) cursordirection |= CD_LEFT;
	if (cdRight->isChecked()) cursordirection |= CD_RIGHT;
}
...
void CursorDirDialog::mouseReleaseEvent (QMouseEvent* _e)
{
	...
	auto inside = [&](const QToolButton* b, bool xAxis) {
		...
		if (xAxis) return x > r.left() && x < r.right();
		return y > r.top() && y < r.bottom();
	};

	CURSORDIRECTION cd = CD_NONE;
	if (inside(cdLeft,  true))  cd |= CD_LEFT;
	if (inside(cdRight, true))  cd |= CD_RIGHT;
	if (inside(cdUp,    false)) cd |= CD_UP;
	if (inside(cdDown,  false)) cd |= CD_DOWN;
	...
	/*  The release is only a "diagonal" when it hits more than one
	    ...
	    those. */
	const bool singleAxis = (cd == CD_LEFT || cd == CD_RIGHT
	                      || cd == CD_UP   || cd == CD_DOWN);
	const QRect upRect    = cdUp   ? cdUp   ->geometry() : QRect();
	const QRect leftRect  = cdLeft ? cdLeft ->geometry() : QRect();
	const bool overCentre = (x > upRect.left()   && x < upRect.right()
	...
	if (cd != CD_NONE && !singleAxis) {
		if (cd & CD_UP)    cdUp   ->setChecked(true);
		if (cd & CD_DOWN)  cdDown ->setChecked(true);
		if (cd & CD_LEFT)  cdLeft ->setChecked(true);
		if (cd & CD_RIGHT) cdRight->setChecked(true);
	}
