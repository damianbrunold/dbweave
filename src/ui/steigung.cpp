...
void TDBWFRM::SpInvert()
{
	...
	if (t2 < t1) return;
	if (schuesse.a < 0 || schuesse.b < 0) return;
...
{
	if (f == GEWEBE || f == AUFKNUEPFUNG) return true;
	if (f == TRITTFOLGE && !frm->trittfolge.einzeltritt) return true;
	return false;
}
...
void TDBWFRM::SteigungInc()
{
	...
	if (!selection.Valid() || !allowedForSteigung(this, selection.feld)) {
		selection = savesel; return;
	}
	IncrementSteigung(selection.begin.i, selection.begin.j,
	                  selection.end.i,   selection.end.j,   selection.feld);
	if (selection.feld != GEWEBE) RecalcGewebe();
	CalcRangeSchuesse();
	CalcRangeKette();
	UpdateRapport();
	selection = savesel;
...
void TDBWFRM::SteigungDec()
{
	...
	DecrementSteigung(selection.begin.i, selection.begin.j,
	                  selection.end.i,   selection.end.j,   selection.feld);
	if (selection.feld != GEWEBE) RecalcGewebe();
	CalcRangeSchuesse();
	CalcRangeKette();
