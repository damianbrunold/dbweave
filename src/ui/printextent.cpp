...
void PrPrinter::CalcKette ()
{
	...
	for (int i = 0; i < data->MAXX1; i++)
		if (!mainfrm->IsEmptyEinzug(i)) { kette.a = i; break; }
	for (int i = data->MAXX1 - 1; i >= 0; i--)
		if (!mainfrm->IsEmptyEinzug(i)) { kette.b = i; break; }
}
...
void PrPrinter::CalcSchuesse ()
{
	...
	for (int j = 0; j < data->MAXY2; j++)
		if (!mainfrm->IsEmptyTrittfolge(j)) { schuesse.a = j; break; }
	for (int j = data->MAXY2 - 1; j >= 0; j--)
		if (!mainfrm->IsEmptyTrittfolge(j)) { schuesse.b = j; break; }
}
