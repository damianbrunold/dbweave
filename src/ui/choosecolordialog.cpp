...
public:
    enum Chan { R, G, B };
    ChannelSwatch(Chan _c, QWidget* _p)
        : QWidget(_p)
        , chan(_c)
    {
        setFixedWidth(18);
    ...
    : QDialog(_parent)
{
    ...
    auto makeCol = [this](const QString& lab, QSlider* sl, QLabel* val, ChannelSwatch::Chan chan) {
...
{
public:
    HueWheel(ChooseHSVDialog* _owner)
        : QWidget(_owner)
        , owner(_owner)
    {
...
{
public:
    SatValPatch(ChooseHSVDialog* _owner)
        : QWidget(_owner)
        , owner(_owner)
    {
...
protected:
    ...
    void mouseReleaseEvent(QMouseEvent* _e) override
    {
        ...
        owner->setHSV(owner->hue(), std::clamp(s, 0, 255), std::clamp(v, 0, 255));
    }
...
int ChooseHSVDialog::hue() const
{
    return slHue->value();
}
int ChooseHSVDialog::sat() const
{
    return slSat->value();
}
int ChooseHSVDialog::val() const
{
    return slVal->value();
}
...
void ChooseHSVDialog::updateAll()
{
    ...
    QPalette pal = preview->palette();
    pal.setColor(QPalette::Window, QColor::fromHsv(hue(), sat(), val()));
    preview->setPalette(pal);
    preview->update();
    wheel->update();
    patch->update();
...
int ChoosePaletteDialog::GetSelectedIndex() const
{
    return canvas->index;
}
