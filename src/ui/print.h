...
public:
    ...
    virtual void PrintRange(const SZ& _kette, const SZ& _schuesse, const SZ& _schaefte,
                            const SZ& _tritte);
...
private:
    /*  Cell rendering primitive shared by einzug/aufknuepfung/
        trittfolge. */
    void PrPaintCell(DARSTELLUNG _d, int _x, int _y, int _xx, int _yy, TColor _col = TColor(0),
                     int _number = -1);
