...
#define RESETSTRONGWIDTH \
	canvas->setPen(savedPen);

/*-----------------------------------------------------------------*/
void PrPrinter::PrintHilfslinien ()
{
	if (!mainfrm->ViewHlines || !mainfrm->ViewHlines->isChecked()) return;
...
void PrPrinter::PrintHilfslinie (Hilfslinie* _h)
{
	...
	case HL_HORZ:
		if (_h->feld == HL_TOP) {
			if (!einzug.visible) break;
			if (_h->pos <= schaefte.a || _h->pos >= schaefte.b) break;
			canvas->setPen(hlcol);
			int y;
			if (mainfrm->toptobottom) y = einzug.y0 + (_h->pos - schaefte.a) * einzug.gh;
			else                      y = einzug.y0 + einzug.height - (_h->pos - schaefte.a) * einzug.gh;
			SETSTRONGWIDTH
			...
			if (_h->pos <= schuesse.a || _h->pos >= schuesse.b) break;
	...
	case HL_VERT:
		if (_h->feld == HL_LEFT) {
			...
			if (_h->pos <= kette.a || _h->pos >= kette.b) break;
			int x;
			if (!rtl) x = gewebe.x0 + (_h->pos - kette.a) * gewebe.gw;
			else      x = gewebe.x0 + gewebe.width - (_h->pos - kette.a) * gewebe.gw;
			SETSTRONGWIDTH
			strongPen.setColor(hlcol);
			canvas->setPen(strongPen);
			if ((mainfrm->GewebeNormal && mainfrm->GewebeNormal->isChecked())
			 || (mainfrm->GewebeNone   && mainfrm->GewebeNone->isChecked()))
				canvas->drawLine(XSCALE(x), YSCALE(gewebe.y0 + pw),
				                 XSCALE(x), YSCALE(gewebe.y0 + gewebe.height - pw));
			if (einzug.visible)
				canvas->drawLine(XSCALE(x), YSCALE(einzug.y0 + pw),
				                 XSCALE(x), YSCALE(einzug.y0 + einzug.height - pw));
			RESETSTRONGWIDTH
		} else {
			if (!trittfolge.visible) break;
			if (_h->pos <= tritte.a || _h->pos >= tritte.b) break;
			int x = trittfolge.x0 + (_h->pos - tritte.a) * trittfolge.gw;
			SETSTRONGWIDTH
			strongPen.setColor(hlcol);
			canvas->setPen(strongPen);
			canvas->drawLine(XSCALE(x), YSCALE(trittfolge.y0 + pw),
			                 XSCALE(x), YSCALE(trittfolge.y0 + trittfolge.height - pw));
			if (einzug.visible)
				canvas->drawLine(XSCALE(x), YSCALE(aufknuepfung.y0 + pw),
				                 XSCALE(x), YSCALE(aufknuepfung.y0 + aufknuepfung.height - pw));
			RESETSTRONGWIDTH
		}
		break;
	}
...
void PrPrinter::PrintKettfarben ()
{
	...
	for (int i = kette.a; i <= kette.b; i++) {
		...
		const int x = rtl ? x0 + (kette.b - i) * kettfarben.gw
		                  : x0 + (i - kette.a) * kettfarben.gw;
		canvas->drawRect(XSCALE(x), YSCALE(y0),
		                 XSCALE(x + kettfarben.gw + 1) - XSCALE(x),
...
void PrPrinter::PrintEinzug ()
{
	...
	for (int i = 0; i < kette.count(); i++) {
		for (int j = 0; j < schaefte.count(); j++) {
			...
			canvas->drawRect(XSCALE(x0 + ii * einzug.gw),
			...
			if (i != 0 && mainfrm->einzug.pos.strongline_x != 0
			 && ((i + kette.a) % mainfrm->einzug.pos.strongline_x) == 0)
				canvas->drawLine(XSCALE(x0 + (ii + 1) * einzug.gw), YSCALE(einzug.y0 + pw),
				                 XSCALE(x0 + (ii + 1) * einzug.gw), YSCALE(einzug.y0 + einzug.height - pw));
		} else {
			if (i != 0 && mainfrm->einzug.pos.strongline_x != 0
			 && ((i + kette.a) % mainfrm->einzug.pos.strongline_x) == 0)
				canvas->drawLine(XSCALE(x0 + ii * einzug.gw), YSCALE(einzug.y0 + pw),
				                 XSCALE(x0 + ii * einzug.gw), YSCALE(einzug.y0 + einzug.height - pw));
		}
	}
	for (int j = 0; j < schaefte.count(); j++) {
		const int jj = ttb ? schaefte.count() - 1 - j : j;
			...
			if (j != 0 && mainfrm->einzug.pos.strongline_y != 0
			 && ((j + schaefte.a) % mainfrm->einzug.pos.strongline_y) == 0)
				canvas->drawLine(XSCALE(einzug.x0 + pw),            YSCALE(y0 - (jj + 1) * einzug.gh),
				                 XSCALE(einzug.x0 + einzug.width - pw), YSCALE(y0 - (jj + 1) * einzug.gh));
		} else {
			if (j != 0 && mainfrm->einzug.pos.strongline_y != 0
			 && ((j + schaefte.a) % mainfrm->einzug.pos.strongline_y) == 0)
				canvas->drawLine(XSCALE(einzug.x0 + pw),            YSCALE(y0 - jj * einzug.gh),
				                 XSCALE(einzug.x0 + einzug.width - pw), YSCALE(y0 - jj * einzug.gh));
		}
	}
	RESETSTRONGWIDTH
}
...
void PrPrinter::PrintBlatteinzug ()
{
	...
	for (int i = kette.a; i <= kette.b; i++) {
		...
		const int x = rtl ? x0 + (kette.b - i) * blatteinzug.gw
		                  : x0 + (i - kette.a) * blatteinzug.gw;
		...
		canvas->drawRect(XSCALE(x), YSCALE(y0 + blatteinzug.height/2 - 1),
		                 XSCALE(x + blatteinzug.gw + 1) - XSCALE(x),
		                 YSCALE(y0 + blatteinzug.height + 1) - YSCALE(y0 + blatteinzug.height/2 - 1));
	}
}
...
void PrPrinter::PrintGewebe ()
{
	...
	for (int i = 0; i < kette.count(); i++) {
		for (int j = 0; j < schuesse.count(); j++) {
			const int ii = rtl ? kette.count() - 1 - i : i;
			if (mainfrm->IsEmptyTrittfolge(j + schuesse.a)
			 || mainfrm->IsEmptyEinzug(i + kette.a)
			 || normal) {
				canvas->setPen(Qt::black);
				canvas->setBrush(Qt::NoBrush);
				canvas->drawRect(XSCALE(x0 + ii * gewebe.gw),
				                 YSCALE(y0 - (j + 1) * gewebe.gh),
				...
				if (i != 0 && mainfrm->gewebe.pos.strongline_x != 0
				 && ((i + kette.a) % mainfrm->gewebe.pos.strongline_x) == 0)
					canvas->drawLine(XSCALE(x0 + (ii + 1) * gewebe.gw), YSCALE(gewebe.y0 + pw),
					                 XSCALE(x0 + (ii + 1) * gewebe.gw), YSCALE(gewebe.y0 + gewebe.height - pw));
			} else {
				if (i != 0 && mainfrm->gewebe.pos.strongline_x != 0
				 && ((i + kette.a) % mainfrm->gewebe.pos.strongline_x) == 0)
					canvas->drawLine(XSCALE(x0 + ii * gewebe.gw), YSCALE(gewebe.y0 + pw),
					                 XSCALE(x0 + ii * gewebe.gw), YSCALE(gewebe.y0 + gewebe.height - pw));
			}
		}
		for (int j = 0; j < schuesse.count(); j++) {
	...
	if (!schlag || !pegplanstyle || mainfrm->einzugunten) {
		for (int i = 0; i < tritte.count(); i++) {
			for (int j = 0; j < schaefte.count(); j++) {
				...
				canvas->setPen(Qt::black);
				canvas->setBrush(Qt::NoBrush);
				canvas->drawRect(XSCALE(x0 + i * aufknuepfung.gw),
				...
				if (!(mainfrm->ViewOnlyGewebe && mainfrm->ViewOnlyGewebe->isChecked())) {
					...
					if (s == AUSHEBUNG)      PrPaintCell(mainfrm->darst_aushebung, _x, _y, _xx, _yy);
					else if (s == ANBINDUNG) PrPaintCell(mainfrm->darst_anbindung, _x, _y, _xx, _yy);
					else if (s == ABBINDUNG) PrPaintCell(mainfrm->darst_abbindung, _x, _y, _xx, _yy);
					else if (s > 0) {
						if (schlag) PrPaintCell(mainfrm->aufknuepfung.darstellung, _x, _y, _xx, _yy);
						else        PrPaintCell(mainfrm->aufknuepfung.darstellung, _x, _y, _xx, _yy,
			...
			if (i != 0 && mainfrm->aufknuepfung.pos.strongline_x != 0
			 && ((i + tritte.a) % mainfrm->aufknuepfung.pos.strongline_x) == 0)
				canvas->drawLine(XSCALE(x0 + i * aufknuepfung.gw), YSCALE(aufknuepfung.y0 + pw),
				                 XSCALE(x0 + i * aufknuepfung.gw), YSCALE(aufknuepfung.y0 + aufknuepfung.height - pw));
		}
		for (int j = 0; j < schaefte.count(); j++) {
			const int jj = mainfrm->toptobottom ? schaefte.count() - 1 - j : j;
			if (mainfrm->toptobottom) {
				if (j != 0 && mainfrm->aufknuepfung.pos.strongline_y != 0
				 && ((j + schaefte.a) % mainfrm->aufknuepfung.pos.strongline_y) == 0)
					canvas->drawLine(XSCALE(aufknuepfung.x0 + pw), YSCALE(y0 - (jj + 1) * aufknuepfung.gh),
					                 XSCALE(aufknuepfung.x0 + aufknuepfung.width - pw), YSCALE(y0 - (jj + 1) * aufknuepfung.gh));
			} else {
				if (j != 0 && mainfrm->aufknuepfung.pos.strongline_y != 0
				 && ((j + schaefte.a) % mainfrm->aufknuepfung.pos.strongline_y) == 0)
					canvas->drawLine(XSCALE(aufknuepfung.x0 + pw), YSCALE(y0 - jj * aufknuepfung.gh),
					                 XSCALE(aufknuepfung.x0 + aufknuepfung.width - pw), YSCALE(y0 - jj * aufknuepfung.gh));
			}
		}
		RESETSTRONGWIDTH
	} else {
...
void PrPrinter::PrintTrittfolge ()
{
	...
	const bool schlag = mainfrm->ViewSchlagpatrone && mainfrm->ViewSchlagpatrone->isChecked();
	const DARSTELLUNG darst = schlag ? mainfrm->schlagpatronendarstellung : mainfrm->trittfolge.darstellung;
	...
	for (int i = 0; i < tritte.count(); i++) {
		for (int j = 0; j < schuesse.count(); j++) {
			canvas->setPen(Qt::black);
			canvas->setBrush(Qt::NoBrush);
			canvas->drawRect(XSCALE(x0 + i * trittfolge.gw),
			                 YSCALE(y0 - (j + 1) * trittfolge.gh),
			...
			if (!(mainfrm->ViewOnlyGewebe && mainfrm->ViewOnlyGewebe->isChecked())) {
				...
				else if (s > 0) PrPaintCell(darst, _x, _y, _xx, _yy, GetRangeColor(s), i);
			}
		}
	}
		...
		if (i != 0 && mainfrm->trittfolge.pos.strongline_x != 0
		 && ((i + tritte.a) % mainfrm->trittfolge.pos.strongline_x) == 0)
			canvas->drawLine(XSCALE(x0 + i * trittfolge.gw), YSCALE(trittfolge.y0 + pw),
			                 XSCALE(x0 + i * trittfolge.gw), YSCALE(trittfolge.y0 + trittfolge.height - pw));
	}
	for (int j = 0; j < schuesse.count(); j++) {
		if (j != 0 && mainfrm->trittfolge.pos.strongline_y != 0
		 && ((j + schuesse.a) % mainfrm->trittfolge.pos.strongline_y) == 0)
			canvas->drawLine(XSCALE(trittfolge.x0 + pw), YSCALE(y0 - j * trittfolge.gh),
			                 XSCALE(trittfolge.x0 + trittfolge.width - pw), YSCALE(y0 - j * trittfolge.gh));
	}
	RESETSTRONGWIDTH
}
