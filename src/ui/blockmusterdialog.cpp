...
protected:
    void paintEvent(QPaintEvent*) override
    {
        ...
        for (int i = 0; i <= mxG; i++)
            p.drawLine(i * dx, 0, i * dx, myG * dy);
        for (int j = 0; j <= myG; j++)
            p.drawLine(0, j * dy, mxG * dx, j * dy);
        ...
        for (int i = 0; i < mxG; i++) {
            for (int j = 0; j < myG; j++) {
                ...
                if (c == 0)
                    continue;
                const QColor col = owner->current == 0 ? QColor(128, 128, 128) : bindungColor(c);
        ...
        if (owner->withRange && owner->mx >= 0 && owner->my >= 0) {
            ...
            p.drawLine(1, (myG - 1 - owner->my) * dy, (owner->mx + 1) * dx,
                       (myG - 1 - owner->my) * dy);
            p.drawLine((owner->mx + 1) * dx, (myG - 1 - owner->my) * dy, (owner->mx + 1) * dx,
                       myG * dy);
        }
    ...
    void keyPressEvent(QKeyEvent* _e) override
    {
        ...
        case Qt::Key_Space: {
            ...
            if ((cursordirection & CD_UP) && cy < myG - 1)
                cy++;
            else if ((cursordirection & CD_DOWN) && cy > 0)
                cy--;
            if ((cursordirection & CD_LEFT) && cx > 0)
                cx--;
            else if ((cursordirection & CD_RIGHT) && cx < mxG - 1)
                cx++;
            update();
            break;
        }
        case Qt::Key_Return:
        case Qt::Key_Enter:
            owner->selectBindung(shift ? (owner->current + 9) % 10 : (owner->current + 1) % 10);
            break;
        case Qt::Key_0:
            owner->selectBindung(0);
            break;
        case Qt::Key_1:
            owner->selectBindung(1);
            break;
        case Qt::Key_2:
            owner->selectBindung(2);
            break;
        case Qt::Key_3:
            owner->selectBindung(3);
            break;
        case Qt::Key_4:
            owner->selectBindung(4);
            break;
        case Qt::Key_5:
            owner->selectBindung(5);
            break;
        case Qt::Key_6:
            owner->selectBindung(6);
            break;
        case Qt::Key_7:
            owner->selectBindung(7);
            break;
        case Qt::Key_8:
            owner->selectBindung(8);
            break;
        case Qt::Key_9:
            owner->selectBindung(9);
            break;
        default:
    ...
    void mouseReleaseEvent(QMouseEvent* _e) override
    {
        ...
        if (i < 0 || j < 0)
            return;
        if (i >= Muster::maxx || j >= Muster::maxy)
            return;
        Muster& m = (*owner->bindungen)[owner->current];
...
/*-----------------------------------------------------------------*/
BlockmusterDialog::BlockmusterDialog(TDBWFRM* _frm, BlockUndo& _undo, PMUSTERARRAY _bindungen,
                                     int& _current, const QString& _caption, bool _withRange)
    : QDialog(_frm)
    , frm(_frm)
    , undo(_undo)
    ...
    , withRange(_withRange)
{
    ...
    connect(timer, &QTimer::timeout, this, [this] {
        if (undoAct)
            undoAct->setEnabled(undo.CanUndo());
        if (redoAct)
            redoAct->setEnabled(undo.CanRedo());
    });
    timer->start(200);
...
void BlockmusterDialog::selectBindung(int _b)
{
    current = _b;
    if (bindungActs[_b])
        bindungActs[_b]->setChecked(true);
    refreshDescription();
    canvas->update();
    refreshUsed();
}
...
{
    QStringList parts;
    parts << (!(*bindungen)[0].IsEmpty() ? QStringLiteral("<span style='color:red'>B</span>")
...
{
    if (_s == 0)
        return char(_current != 0 ? _current : 1);
    return 0;
}
        ...
        if (pData[i - kette.a] > 0) {
            if (einzugZ) {
                for (int k = 0; k < _count; k++) {
                    if (i * _count + k >= Data->MAXX1)
                        break;
                    if ((pData[i - kette.a] - 1) * _count + k + 1 >= Data->MAXY1)
                        ExtendSchaefte();
                    einzug.feld.Set(i * _count + k,
                                    short((pData[i - kette.a] - 1) * _count + k + 1));
                }
            } else {
                for (int k = 0; k < _count; k++) {
                    if ((i + 1) * _count - 1 - k >= Data->MAXX1)
                        break;
                    if ((pData[i - kette.a] - 1) * _count + k + 1 >= Data->MAXY1)
                        ExtendSchaefte();
                    einzug.feld.Set((i + 1) * _count - 1 - k,
                                    short((pData[i - kette.a] - 1) * _count + k + 1));
                }
            }
        }
        ...
        for (int j = j1; j <= j2; j++) {
            ...
            for (int ii = _x * i; ii < _x * (i + 1); ii++)
                for (int jj = _y * j; jj < _y * (j + 1); jj++)
                    aufknuepfung.feld.Set(ii, jj,
                                          blockmuster[int(bindung)].Get(ii - _x * i, jj - _y * j));
        }
    }
    delete[] pData;
}
