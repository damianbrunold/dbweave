...
void __fastcall Hilfslinien::Delete (int _index)
{
	...
	dbw3_assert (_index <= last && _index < max);
	if (_index > last) return;
	for (int i = _index + 1; i <= last; i++) list[i - 1] = list[i];
	last--;
}
...
void __fastcall Hilfslinien::Delete (Hilfslinie* _hline)
{
	...
	for (int i = 0; i <= last && i < max; i++) {
		if (list[i].typ == _hline->typ &&
		    list[i].feld == _hline->feld &&
...
static void __fastcall _DrawHilfslinie (TDBWFRM* frm, Hilfslinie* _hline)
{
	...
	case HL_HORZ: {
		if (_hline->feld == HL_TOP) {
			if (!viewEinzug) break;
			if (frm->einzug.gh <= 0) break;
			if (_hline->pos <= frm->scroll_y1 ||
			    _hline->pos >= frm->scroll_y1 + frm->hvisible) break;
			int y;
			if (frm->toptobottom) {
			...
			p->drawLine (frm->einzug.pos.x0 + 1, y,
			             frm->einzug.pos.x0 + frm->einzug.pos.width, y);
			...
			p->drawLine (frm->hlinevert1.x0 + 5, y,
			             frm->hlinevert1.x0 + frm->hlinevert1.width, y);
		} else {
			if (frm->gewebe.gh <= 0) break;
			if (_hline->pos <= frm->scroll_y2 ||
			    _hline->pos >= frm->scroll_y2 + frm->gewebe.pos.height / frm->gewebe.gh) break;
			int y = frm->gewebe.pos.y0 + frm->gewebe.pos.height
			      - (_hline->pos - frm->scroll_y2) * frm->gewebe.gh;
			if (gewebeNormal || gewebeNone) {
				p->drawLine (frm->gewebe.pos.x0 + 1, y,
				             frm->gewebe.pos.x0 + frm->gewebe.pos.width, y);
			}
			if (viewTrittfolge) {
				p->drawLine (frm->trittfolge.pos.x0 + 1, y,
				             frm->trittfolge.pos.x0 + frm->trittfolge.pos.width, y);
			}
			p->drawLine (frm->hlinevert2.x0 + 5, y,
			             frm->hlinevert2.x0 + frm->hlinevert2.width, y);
		}
		break;
	}
	case HL_VERT: {
		if (_hline->feld == HL_LEFT) {
			if (frm->gewebe.gw <= 0) break;
			if (_hline->pos <= frm->scroll_x1 ||
			    _hline->pos >= frm->scroll_x1 + frm->gewebe.pos.width / frm->gewebe.gw) break;
			int x;
			if (frm->righttoleft) {
			...
			if (gewebeNormal || gewebeNone) {
				p->drawLine (x, frm->gewebe.pos.y0 + 1,
				             x, frm->gewebe.pos.y0 + frm->gewebe.pos.height);
			}
			if (viewEinzug) {
				p->drawLine (x, frm->einzug.pos.y0 + 1,
				             x, frm->einzug.pos.y0 + frm->einzug.pos.height);
			}
			p->drawLine (x, frm->hlinehorz1.y0 + 1,
			             x, frm->hlinehorz1.y0 + frm->hlinehorz1.height - 4);
		} else {
			if (!viewTrittfolge) break;
			if (frm->trittfolge.gw <= 0) break;
			if (_hline->pos <= frm->scroll_x2 ||
			    _hline->pos >= frm->scroll_x2 + frm->wvisible) break;
			int x = frm->trittfolge.pos.x0 + (_hline->pos - frm->scroll_x2) * frm->trittfolge.gw;
			p->drawLine (x, frm->trittfolge.pos.y0 + 1,
			             x, frm->trittfolge.pos.y0 + frm->trittfolge.pos.height);
			if (viewEinzug && !viewSchlagpat) {
				p->drawLine (x, frm->aufknuepfung.pos.y0 + 1,
				             x, frm->aufknuepfung.pos.y0 + frm->aufknuepfung.pos.height);
			}
			p->drawLine (x, frm->hlinehorz2.y0 + 1,
			             x, frm->hlinehorz2.y0 + frm->hlinehorz2.height - 4);
		}
		break;
	}
	case HL_NONE:
...
void __fastcall TDBWFRM::DrawHilfslinien()
{
	...
	for (int i = 0; i < count; i++) {
		...
		if (line) _DrawHilfslinie (this, line);
	}
}
