...
void __fastcall TDBWFRM::LoadUserdefMenu ()
{
	...
	if (MenuWeitere) MenuWeitere->menuAction()->setVisible(menuvisible);
}
	...
	if (userdef[_i].sizex != 0 && kbd_field == GEWEBE) {
		...
		for (int i = x; i < xx; i++) {
			for (int j = y; j < yy; j++) {
				...
				if (s != QChar('k')) gewebe.feld.Set(i, j, char(s.unicode() - 'k'));
		...
		if (gewebe.gw > 0 &&
		    selection.end.i >= scroll_x1 + gewebe.pos.width / gewebe.gw)
			selection.end.i = scroll_x1 + gewebe.pos.width / gewebe.gw - 1;
		if (gewebe.gh > 0 &&
		    selection.end.j >= scroll_y2 + gewebe.pos.height / gewebe.gh)
	...
	if (selection.Valid() && selection.feld == GEWEBE) {
		...
		if (i >= 0 && i < MAXUSERDEF) {
			...
			if (!descr.isEmpty()) {
				...
				settings.Save(AnsiString("Data"),        encodeRect(this,
