	...
	: QDialog(_parent)
{
	...
	auto* warn = new QLabel(
	    QStringLiteral("<i>Note: the executable was built without "
	...
	connect(cbInterface, qOverload<int>(&QComboBox::currentIndexChanged),
	        this, &LoomOptionsDialog::onInterfaceChanged);
...
int LoomOptionsDialog::port  () const { return cbPort->currentIndex() + 1; }
int LoomOptionsDialog::delay () const { return spDelay->value(); }

void LoomOptionsDialog::setInterface (LOOMINTERFACE _i) { cbInterface->setCurrentIndex(int(_i)); }
void LoomOptionsDialog::setPort      (int _p)           { cbPort->setCurrentIndex(_p - 1);       }
void LoomOptionsDialog::setDelay     (int _d)           { spDelay->setValue(_d);                 }
