...
/*-----------------------------------------------------------------*/
void PrPrinter::PrPaintCell (DARSTELLUNG _d, int _x, int _y, int _xx, int _yy,
                             TColor _col, int _number)
{
	int penwidth = 0;
	...
	case STRICH:
		canvas->drawLine(XSCALE((_x+_xx)/2),   YSCALE(_yy-dy),
		                 XSCALE((_x+_xx)/2),   YSCALE(_y+dy+1));
		canvas->drawLine(XSCALE((_x+_xx)/2+1), YSCALE(_yy-dy),
		                 XSCALE((_x+_xx)/2+1), YSCALE(_y+dy+1));
		break;

	case KREUZ:
		canvas->drawLine(XSCALE(_x+dx), YSCALE(_yy-dy),
		                 XSCALE(_xx-dx), YSCALE(_y+dy));
		canvas->drawLine(XSCALE(_x+dx), YSCALE(_y+dy),
		                 XSCALE(_xx-dx), YSCALE(_yy-dy));
		break;
	...
	case SMALLKREIS: {
		...
		if (small.width() > 2) small.setWidth(small.width()/2);
		canvas->setPen(small);
		canvas->setBrush(Qt::NoBrush);
	...
	case SMALLKREUZ: {
		...
		const int sdx = dx + (_xx-_x)/10;
		const int sdy = dy + (_yy-_y)/10;
		canvas->drawLine(XSCALE(_x+sdx), YSCALE(_yy-sdy),
		                 XSCALE(_xx-sdx), YSCALE(_y+sdy));
		canvas->drawLine(XSCALE(_x+sdx), YSCALE(_y+sdy),
		                 XSCALE(_xx-sdx), YSCALE(_yy-sdy));
		break;
	}
...
void PrPrinter::PrintGewebeNone (int _i, int _j, int _x, int _y, int _xx, int _yy)
{
	...
	canvas->setBrush(Qt::NoBrush);
	canvas->drawRect(XSCALE(_x), YSCALE(_y),
	                 XSCALE(_xx) - XSCALE(_x), YSCALE(_yy) - YSCALE(_y));
}
...
void PrPrinter::PrintGewebeNormal (int _i, int _j, int _x, int _y, int _xx, int _yy)
{
	...
	if (s == AUSHEBUNG)      PrPaintCell(mainfrm->darst_aushebung, _x, _y, _xx, _yy);
		...
		canvas->fillRect(QRect(XSCALE(_x), YSCALE(_y),
		                       XSCALE(_xx+1) - XSCALE(_x),
...
void PrPrinter::PrintGewebeFarbeffekt (int _i, int _j, int _x, int _y, int _xx, int _yy)
{
	...
	if (mainfrm->sinkingshed) hebung = !hebung;
	const QColor col = hebung
	    ? palCol(mainfrm->kettfarben.feld.Get(_i + kette.a))
	    : palCol(mainfrm->schussfarben.feld.Get(_j + schuesse.a));
	canvas->fillRect(QRect(XSCALE(_x), YSCALE(_y),
	                       XSCALE(_xx+1) - XSCALE(_x),
	                       YSCALE(_yy+1) - YSCALE(_y)),
	                 col);
	if (mainfrm->fewithraster) PrintGewebeNone(_i, _j, _x, _y, _xx, _yy);
}
...
void PrPrinter::PrintGewebeSimulation (int _i, int _j, int _x, int _y, int _xx, int _yy)
{
	...
	if (dw == 0 && std::abs(_xx - _x) > 2) dw = 1;
	if (dh == 0 && std::abs(_yy - _y) > 2) dh = 1;
	...
	if (hebung) {
		canvas->fillRect(QRect(XSCALE(_x+dw), YSCALE(_y),
		                       XSCALE(_xx-dw) - XSCALE(_x+dw),
		                       YSCALE(_yy)    - YSCALE(_y)), kettCol);
		canvas->fillRect(QRect(XSCALE(_x),    YSCALE(_y+dh),
		                       XSCALE(_x+dw) - XSCALE(_x),
		                       YSCALE(_yy-dh)- YSCALE(_y+dh)), schuCol);
		canvas->fillRect(QRect(XSCALE(_xx-dw),YSCALE(_y+dh),
		                       XSCALE(_xx)   - XSCALE(_xx-dw),
		                       YSCALE(_yy-dh)- YSCALE(_y+dh)), schuCol);
		QPen p(Qt::black); p.setWidth(dw > 1 ? dw/3 : 1);
		canvas->setPen(p);
		canvas->drawLine(XSCALE(_xx-dw), YSCALE(_y+dh), XSCALE(_xx-dw), YSCALE(_yy-dh));
	} else {
		canvas->fillRect(QRect(XSCALE(_x),    YSCALE(_y+dh),
		                       XSCALE(_xx)   - XSCALE(_x),
		                       YSCALE(_yy-dh)- YSCALE(_y+dh)), schuCol);
		canvas->fillRect(QRect(XSCALE(_x+dw), YSCALE(_y),
		                       XSCALE(_xx-dw)- XSCALE(_x+dw),
		                       YSCALE(_y+dh) - YSCALE(_y)), kettCol);
		canvas->fillRect(QRect(XSCALE(_x+dw), YSCALE(_yy-dh),
		                       XSCALE(_xx-dw)- XSCALE(_x+dw),
		                       YSCALE(_yy)   - YSCALE(_yy-dh)), kettCol);
		QPen p(Qt::black); p.setWidth(dh > 1 ? dh/3 : 1);
		canvas->setPen(p);
