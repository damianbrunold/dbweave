...
std::uint32_t LoomDialog::computeShafts() const
{
    ...
    if (currentPosition < 0 || currentPosition >= Data->MAXY2)
        return 0;

    if (schlag) {
        for (int i = 0; i < maxShaft; i++)
            if (i < Data->MAXX2 && frm->trittfolge.feld.Get(i, currentPosition) > 0)
...
void LoomDialog::onStart()
{
    ...
    while (!stopRequested && controller && !controller->IsAborted()) {
        weaveOne();
        if (stopRequested || controller->IsAborted())
            break;
        if (!loop && !firstPass && currentKlammer == startKl && currentPosition == startPos
            && currentRepetition == startRep)
            break;
        firstPass = false;
        if (++guard > 100000)
            break; /*  safety valve */
...
void LoomDialog::onStep()
{
    ...
    controller->Terminate();
    if (bGotoLast)
        bGotoLast->setEnabled(lastKlammer >= 0);
}
...
{
    pushKlammersToFrm();
    if (_index < 0 || _index >= kKlammerCount)
        return;
    if (frm->klammern[_index].repetitions == 0)
        return;
