	...
	, frm(_frm)
{
	...
	connect(categories, &QListWidget::currentRowChanged,
	        this, &EntwurfsinfoDialog::onCategoryChanged);
...
{
	switch (_row) {
	case 0: onAusmasse();      break;
	case 1: onFarben();        break;
	case 2: onFlottierungen(); break;
	case 3: onLitzen();        break;
	}
}
...
void EntwurfsinfoDialog::insertHeading (const QString& _heading)
{
	...
	if (!text->document()->isEmpty()) c.insertBlock();
...
void EntwurfsinfoDialog::buildAusmasse ()
{
	...
	if (frm->kette.a==-1 || frm->kette.b==-1 ||
	    frm->schuesse.a==-1 || frm->schuesse.b==-1)
		return;
	...
	if (hebungen<senkungen)
		ausmasse << QStringLiteral("The pattern is weft sided (") +
		            fmt3g(100.0*senkungen/total) +
		            QStringLiteral("% sinking binding points)");
	else if (hebungen>senkungen)
		ausmasse << QStringLiteral("The pattern is warp sided (") +
...
void EntwurfsinfoDialog::buildFarben ()
{
	...
	for (int i=frm->kette.a; i<=frm->kette.b; i++) {
		...
		if (!coltable[c]) { farbcount++; kfarbcount++; }
		coltable[c] = true;
		ktable[c]++;
	}
	for (int j=frm->schuesse.a; j<=frm->schuesse.b; j++) {
		unsigned char c = frm->schussfarben.feld.Get(j);
		if (!coltable[c]) { farbcount++; sfarbcount++; }
		coltable[c] = true;
		stable[c]++;
	}
	...
	emitPalette("Warp colors:", ktable,
	            frm->kette.b-frm->kette.a+1, " warp ends  (");
	emitPalette("Weft colors:", stable,
	            frm->schuesse.b-frm->schuesse.a+1, " weft picks  (");
}
...
void EntwurfsinfoDialog::buildLitzen ()
{
	...
	for (int i=0; i<Data->MAXX1; i++)
		if (frm->einzug.feld.Get(i)!=0) litzennr++;
	...
	for (int j=0; j<Data->MAXY1; j++) {
		if (!frm->freieschaefte[j]) {
			...
			int count = 0;
			for (int i=0; i<Data->MAXX1; i++)
				if (frm->einzug.feld.Get(i)==j+1) count++;
			litzen << QStringLiteral("Shaft %1:  %2 Heddles  (%3%)")
			          .arg(schaft).arg(count)
...
/*-----------------------------------------------------------------*/
static void buildFloatDir (const QStringList::iterator&,
                           QStringList& /*unused*/) {}
...
void EntwurfsinfoDialog::buildFlottierungen ()
{
	...
	try {
		...
		for (int i=0; i<maxflot; i++) table[i] = 0;
		int biggest = 0;
		int count = 0;
		for (int j=frm->schuesse.a; j<=frm->schuesse.b; j++) {
...
{
	for (const QString& line : _lines) _text->append(line);
}

void EntwurfsinfoDialog::onAusmasse (bool _clear)
{
	if (_clear) text->clear();
	if (!read_ausmasse) { buildAusmasse(); read_ausmasse = true; }
...
{
	if (_clear) text->clear();
	if (!read_farben) { buildFarben(); read_farben = true; }
...
void EntwurfsinfoDialog::onLitzen (bool _clear)
{
	...
	if (_clear) text->clear();
	if (!read_litzen) { buildLitzen(); read_litzen = true; }
...
void EntwurfsinfoDialog::onFlottierungen (bool _clear)
{
	QApplication::setOverrideCursor(Qt::WaitCursor);
	if (_clear) text->clear();
	if (!read_flottierungen) { buildFlottierungen(); read_flottierungen = true; }
...
void EntwurfsinfoDialog::onPrint ()
{
	...
	text->clear();
	insertHeader();
	insertHeading(QStringLiteral("General"));      onAusmasse(false);
	insertHeading(QStringLiteral("Colors"));       onFarben(false);
	insertHeading(QStringLiteral("Floats"));       onFlottierungen(false);
	insertHeading(QStringLiteral("Heddles"));      onLitzen(false);
...
void EntwurfsinfoDialog::onExport ()
{
	...
	const QString fn = QFileDialog::getSaveFileName(
	    this,
	    QStringLiteral("Save pattern info as"),
	    suggested,
